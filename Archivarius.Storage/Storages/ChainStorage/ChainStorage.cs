...
namespace Archivarius.Storage
{
        ...
        where TData : class, IDataStruct
    {
        ...
        {
            _index ??= await _storage.GetVersionedStruct<IndexData>(_rootPath.File("index"));
        ...
        {
            await _storage.SetVersionedStruct(_rootPath.File("index"), _index!.Value);
        }
        ...
        private async ValueTask<PackData> GetPack_Unsafe(int packId)
        {
            ...
            {
                string packName = string.Format(_index!.Value.PackName, packId);
        ...
        {
            string elementName = string.Format(_index!.Value.ElementName, elementId);
